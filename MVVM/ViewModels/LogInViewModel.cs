...
namespace PortBridgeShipping.MVVM.ViewModels
{
    public class LogInViewModel : ObservableObject
    {
        ...
        {
            // Simple registration flow: create user if not exists
            if (_usersService.CreateUser(Username, Password))
            {
                Message = "User created. You can now log in.";
                // keep password so user can press Login; do not auto-login
            }
            else
            {
                Message = "User exists or invalid data.";
            }
        }
    }
}
